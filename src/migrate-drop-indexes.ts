/**
 * One-time migration: drops stale indexes and ensures required indexes exist.
 * Usage: npx ts-node -r tsconfig-paths/register src/migrate-drop-indexes.ts
 */
...
async function run() {
  ...
  // Drop stale unique index on students.rollNo
  try {
  ...
  // students.dni — unique + sparse (sparse allows multiple null values)
  try {
    await db.collection('students').createIndex(
      { dni: 1 },
      { unique: true, sparse: true, name: 'dni_1' },
    );
